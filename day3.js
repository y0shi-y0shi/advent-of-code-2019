// Generate coordinates. Then find repeating points
...
var toCoordinates = function (path) {
    var directions = path.split(',');
    var coordinates = [];
    var point = [0, 0];
    directions.forEach(function (direction) {
        if (direction.slice(0, 1) == 'R') {
            for (var i = 0; i < +direction.slice(1); i++) {
                point[1]++;
                coordinates.push(__spreadArrays(point));
            }
        }
...
var line1 = toCoordinates(path1);
var line2 = toCoordinates(path2);
...
var locations = (intersection(line1, line2));
var manhattanDistance = function (locations) {
    var result = Infinity;
...
var result = manhattanDistance(locations);
console.log(result);
